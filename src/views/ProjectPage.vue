<template>
  <article class="project">
    <link v-if="!projectPass.font.local" :href="projectPass.font.url" rel="stylesheet">
    <load-project v-if="loading"></load-project>
    <div v-if="show" class="stem" :class="['effect-blur', loadImageShow ? 'effect-blur--on' : '']">
      <div class="stem__color"></div>
      <div class="stem__bg"></div>
    </div>
    <a href="#" v-on:click.prevent="hideProject() + !show" class="icon-close" v-if="show">CLOSE</a>
    <transition name="grow">
      <header v-if="show" class="project__header" :class="['effect-blur', loadImageShow ? 'effect-blur--on' : '']" :style="{ backgroundColor: projectPass.colors[0] }">
        <h1 v-text="projectPass.company" :style="{ color: projectPass.colors[1] }"></h1>
        <h2 v-text="projectPass.project" :style="{ color: projectPass.colors[2] }"></h2>
        <a v-if="projectPass.link.active" :href="'//' + projectPass.link.url" target="_blank" class="project__header__link" v-text="projectPass.link.url"></a>
        <span v-else class="project__header__link" v-text="projectPass.link.url"></span>
      </header>
    </transition>

    <transition name="grow">
      <div v-if="show" class="project__body"  :class="['effect-blur', loadImageShow ? 'effect-blur--on' : '']">
        <section class="project__body__info js-elementTop" v-bind:class="{ fixed: isFixed }">
          <ul>
            <li>
              <legend>Project</legend>
              <p v-text="projectPass.name"></p>
            </li>
            <li>
              <legend>Company</legend>
              <p v-text="projectPass.company"></p>
            </li>
            <li>
              <legend>Role</legend>
              <p v-for="role in projectPass.roles" :key="role.key" v-text="role"></p>
            </li>
            <li>
              <legend>Date Released</legend>
              <p v-text="projectPass.released"></p>
            </li>
          </ul>
        </section>
        <section class="project__body__description">
          <div class="project__body__description__block">
            <div class="project__body__description__block__content">
            </div>
            <i class="stem__sprite stem__sprite--space stem__sprite--space-top"></i>
          </div>
          <div class="project__body__description__block">
            <i class="stem__sprite stem__sprite--research"></i>
            <i class="stem__sprite stem__sprite--space"></i>
              <div class="project__body__description__block__content">
                <h3 class="eb-type-h3">Introduction</h3>
                <p v-text="projectPass.body"></p>
              </div>
          </div>
          <div class="project__body__description__block">
            <i class="stem__sprite stem__sprite--architecture"></i>
            <i class="stem__sprite stem__sprite--space"></i>
              <div class="project__body__description__block__content">
                <h3 class="eb-type-h3">Design</h3>
                <legend>Color Palette</legend>
                <ul class="project__palette">
                  <li v-for="color in projectPass.colors" :key="color.key" :style="{ backgroundColor: color }">
                    <span v-text="color"></span>
                  </li>
                </ul>
                <legend>Typography</legend>
                <ul class="project__type" :style="{ fontFamily: projectPass.font.family, fontWeight: projectPass.font.weight }">
                  <li v-text="projectPass.font.face"></li>
                  <li>ABCDEFGHIJKLMNOPQRSTVWXYZ</li>
                  <li>abcdefghijklmnopqrstvwxyz</li>
                  <li>123456789</li>
                </ul>
              </div>
          </div>
          <div class="project__body__description__block">
            <i class="stem__sprite stem__sprite--code"></i>
            <i class="stem__sprite stem__sprite--space"></i>
              <div class="project__body__description__block__content">
                <h3 class="eb-type-h3">Code Stack</h3>
                <ul class="project__stack">
                  <li v-for="stack in projectPass.stack" :key="stack.key">
                    <svg :class="stack.icon">
                      <use v-bind="{'xlink:href':'#'+ stack.icon}"></use>
                    </svg>
                    <p v-text="stack.title"></p>
                  </li>
                </ul>
              </div>
          </div>
          <div class="project__body__description__block">
            <i class="stem__sprite stem__sprite--design"></i>
            <i class="stem__sprite stem__sprite--space"></i>
            <div class="project__body__description__block__content">
              <h3 class="eb-type-h3">Screens</h3>
              <ul class="project__screens">
                <li v-for="screen in projectPass.screens" :key="screen.key">
                  <a v-on:click.prevent="loadImageSend(screen)" :href="screen" target="_blank">
                    <img :src="screen" alt="">
                  </a>
                </li>
              </ul>
            </div>
            <i class="stem__sprite stem__sprite--end"></i>
            <a href="#" v-on:click.prevent="toTop"> Back to Top</a>
          </div>
        </section>
      </div>
    </transition>
    <load-image v-if="loadImageShow" :imgSrc="loadImageSrc"  v-on:loadImageClose="loadImageHide"></load-image>
    <svg xmlns="http://www.w3.org/2000/svg" style="display: none;">
      <symbol id="icon-sass" viewBox="0 0 300 300">
        <path d="M255.21,157.95a60.35,60.35,0,0,0-26.55,6.18c-2.75-5.44-5.49-10.19-6-13.73-.51-4.12-1.14-6.64-.51-11.56s3.49-11.9,3.49-12.47-.63-3-6.52-3.09-11,1.14-11.56,2.69a65.71,65.71,0,0,0-2.46,8.75c-1,5.38-11.79,24.43-17.91,34.44-2-3.89-3.72-7.32-4.06-10.07-.51-4.12-1.14-6.64-.51-11.56s3.49-11.9,3.49-12.47-.63-3-6.52-3.09-11,1.14-11.56,2.69-1.2,5.21-2.46,8.75-15.51,35.36-19.23,43.66c-1.89,4.23-3.55,7.61-4.75,9.9s-.06.17-.17.4l-1.6,3v.06c-.8,1.43-1.66,2.8-2.06,2.8s-.86-3.83.11-9.1c2.12-11,7.27-28.27,7.21-28.89,0-.29,1-3.32-3.32-4.86s-5.66,1-6,1-.63.92-.63.92S139.8,143,126.3,143c-8.47,0-20.14,9.21-25.92,17.62-3.66,2-11.44,6.24-19.68,10.76-3.15,1.72-6.41,3.55-9.5,5.21l-.63-.69C54.2,158.4,23.94,146,25.19,122.59c.46-8.53,3.43-31,58.13-58.25C128.13,42,164,48.15,170.24,61.76c8.87,19.45-19.17,55.62-65.74,60.82-17.74,2-27.06-4.86-29.41-7.44s-2.8-2.8-3.72-2.29c-1.49.8-.57,3.2,0,4.63,1.37,3.6,7.09,10,16.82,13.22,8.52,2.8,29.35,4.35,54.53-5.38,28.21-10.93,50.24-41.25,43.77-66.6-6.58-25.8-49.38-34.27-89.83-19.91-24.09,8.58-50.18,22-68.95,39.54C5.4,99.18,1.85,117.38,3.34,124.93c5.21,26.95,42.34,44.52,57.22,57.5-.74.4-1.43.8-2.06,1.14-7.44,3.66-35.76,18.48-42.86,34.16-8,17.74,1.26,30.5,7.44,32.21,19.11,5.32,38.68-4.23,49.26-20s9.27-36.16,4.41-45.49l-.17-.34,5.84-3.43c3.78-2.23,7.5-4.29,10.76-6.07-1.83,5-3.15,10.87-3.83,19.45-.8,10.07,3.32,23.12,8.75,28.27a9.69,9.69,0,0,0,7,2.29c6.29,0,9.15-5.21,12.3-11.44,3.89-7.61,7.32-16.42,7.32-16.42s-4.29,23.86,7.44,23.86c4.29,0,8.58-5.55,10.53-8.41v.06s.11-.17.34-.57.47-.72.69-1.09v-.11c1.72-3,5.55-9.78,11.27-21.06,7.38-14.53,14.48-32.73,14.48-32.73a104.44,104.44,0,0,0,2.8,11.79c1.26,4.35,4,9.1,6.12,13.73-1.72,2.4-2.75,3.78-2.75,3.78l.06.06c-1.37,1.83-2.92,3.78-4.52,5.72-5.84,7-12.82,14.93-13.73,17.22-1.09,2.69-.86,4.69,1.26,6.29,1.54,1.14,4.29,1.37,7.21,1.14A32.12,32.12,0,0,0,176.71,214a37.25,37.25,0,0,0,9.27-4.86c5.72-4.23,9.21-10.24,8.87-18.25A36.7,36.7,0,0,0,191.47,178c.51-.74,1-1.49,1.54-2.29A241.53,241.53,0,0,0,209,148a104.44,104.44,0,0,0,2.8,11.79c1.09,3.72,3.26,7.78,5.21,11.79-8.47,6.92-13.79,14.93-15.62,20.2-3.38,9.73-.74,14.13,4.23,15.16,2.23.46,5.44-.57,7.84-1.6a35.52,35.52,0,0,0,9.9-5.09c5.72-4.23,11.21-10.13,10.93-18.08a34.66,34.66,0,0,0-2.46-10.7c7.21-3,16.54-4.69,28.38-3.26,25.46,3,30.5,18.88,29.52,25.52s-6.29,10.3-8.07,11.44-2.35,1.49-2.17,2.29c.23,1.2,1,1.14,2.57.92,2.12-.34,13.39-5.44,13.85-17.68.69-15.74-14.25-32.9-40.74-32.73ZM58.84,224.15c-8.41,9.21-20.25,12.7-25.29,9.73-5.44-3.15-3.32-16.71,7-26.49,6.29-6,14.48-11.44,19.85-14.82,1.2-.74,3-1.83,5.21-3.15a6.1,6.1,0,0,1,.57-.34c.4-.23.86-.51,1.32-.8,3.83,14,.17,26.21-8.7,35.88Zm61.51-41.83c-2.92,7.15-9.1,25.52-12.82,24.49-3.2-.86-5.15-14.76-.63-28.49,2.29-6.92,7.15-15.16,10-18.37,4.64-5.15,9.67-6.87,10.93-4.75C129.33,157.95,122.18,177.86,120.35,182.32Zm50.75,24.26c-1.26.63-2.4,1.09-2.92.74s.51-1.09.51-1.09,6.35-6.81,8.87-10c1.43-1.83,3.15-3.95,5-6.35v.69c0,8.24-7.9,13.73-11.44,16Zm39.14-8.93c-.92-.69-.8-2.8,2.29-9.44a35.38,35.38,0,0,1,8.7-11.21,14.15,14.15,0,0,1,.86,4.92C222,192.22,214.7,196.05,210.23,197.65Z"/>
      </symbol>
      <symbol id="icon-sketch" viewBox="0 0 300 300">
        <polygon points="285.87 97.32 239.43 132.07 206.4 82.59 253.13 60.6 285.87 97.32 285.87 97.32"/><polygon points="222.38 137.37 77.41 137.37 106.26 86.12 192.67 86.33 222.38 137.37"/><polygon points="107.42 33.52 195.88 33.52 239.43 52.73 195.69 71.78 107.24 71.78 63.94 52.74 107.42 33.52 107.42 33.52"/><polygon points="49.28 60.6 93.1 82.56 61.72 131.1 16.54 97.32 49.28 60.6 49.28 60.6"/><polygon points="33.38 122.91 61.93 146.98 129.86 258.76 33.38 122.91 33.38 122.91"/><polygon points="81.37 153.27 219.02 153.27 151.25 278.15 81.37 153.27 81.37 153.27"/><polygon points="172.55 258.76 240.48 146.98 269.03 122.91 172.55 258.76"/><path style="fill: #F1F1F1"   d="M255.74,48.47,204.39,25.87H97.91l-51,23L4.41,96.05,151.25,291.33,298.1,96.06,255.74,48.47Zm30.12,48.86-46.44,34.75-33-49.48,46.73-22,32.74,36.72Zm-63.49,40h-145l28.85-51.25,86.41.21Zm-115-103.85h88.46l43.55,19.21L195.69,71.78H107.24l-43.3-19,43.48-19.22ZM49.28,60.6l43.82,22L61.72,131.1,16.54,97.32,49.28,60.6Zm-15.9,62.31L61.93,147l67.93,111.78L33.38,122.91Zm48,30.36H219L151.25,278.15,81.37,153.27Zm91.18,105.49L240.48,147,269,122.91Z"/>
      </symbol>
      <symbol id="icon-vue" viewBox="0 0 300 300">
        <path d="M240,19.55h60l-150.12,259L-.24,19.55H114.6l35.28,60,34.53-60Z"/><path d="M-.24,19.55l150.12,259L300,19.55H240L149.88,174.92,59.06,19.55Z"/><path style="fill: #F1F1F1" d="M59.06,19.55l90.82,156.12L240,19.55H184.41l-34.53,60-35.28-60Z"/>
      </symbol>
      <symbol id="icon-gulp" viewBox="0 0 300 300">
        <path  d="M103.34,86.49a9.63,9.63,0,0,0,1.92-6.33,21.28,21.28,0,0,0-.34-3.62,9.62,9.62,0,0,0-1.62-3.79q-5.58-8.38-12-12.14a25.9,25.9,0,0,0-13.26-3.74,30.42,30.42,0,0,0-12,2.59,55.32,55.32,0,0,0-12,7.18,84.71,84.71,0,0,0-11.6,11,125.26,125.26,0,0,0-10.66,14,153,153,0,0,0-9.26,16.1A165.9,165.9,0,0,0,15,125.11,142.88,142.88,0,0,0,9.79,143a107.9,107.9,0,0,0-2.44,17.58,7.49,7.49,0,0,0-.08,1.07v1.16a38.2,38.2,0,0,0,1.2,9.46,27.43,27.43,0,0,0,3.69,8.48,19.92,19.92,0,0,0,6.29,6.07,17.18,17.18,0,0,0,9.08,2.32,23.34,23.34,0,0,0,9.56-2.23A56,56,0,0,0,47,181.2a93.19,93.19,0,0,0,9.56-7.94q4.68-4.42,8.67-8.88-1.36,4.19-3.05,9.5c-1.13,3.55-2.19,7-3.16,10.26s-1.82,6.26-2.52,8.88a32.45,32.45,0,0,0-1.13,5.36,12.32,12.32,0,0,0-.08,1.33V201q0,4.28,1.09,5.89a3.94,3.94,0,0,0,3.5,1.61,4.59,4.59,0,0,0,2.79-1,12,12,0,0,0,2.49-2.59,21.35,21.35,0,0,0,2-3.3,18.06,18.06,0,0,0,1.21-3.17c.1-.3.38-1.25.83-2.85s1-3.61,1.69-6,1.44-5,2.3-8,1.74-5.95,2.68-9,1.83-6.07,2.71-9,1.69-5.61,2.44-8,1.39-4.35,1.92-5.93A16.76,16.76,0,0,1,84,146.81h0c.9-2.62,1.83-5.14,2.77-7.53s1.82-4.8,2.6-7.18a64.23,64.23,0,0,0,1.92-7.32,40.65,40.65,0,0,0,.75-8,7.48,7.48,0,0,0-.49-2.81,5.27,5.27,0,0,0-1.32-2,5.14,5.14,0,0,0-1.88-1.12,6.56,6.56,0,0,0-2.11-.36,7.45,7.45,0,0,0-4.26,1.43,10,10,0,0,0-3.35,4.82l-7,21A43.56,43.56,0,0,1,67.75,144c-1.7,2.41-3.67,4.92-5.87,7.54s-4.59,5.2-7.15,7.76a93.48,93.48,0,0,1-7.65,6.87,47.11,47.11,0,0,1-7.34,4.91,13.84,13.84,0,0,1-6.32,1.87,6.08,6.08,0,0,1-4.1-1.38A9.49,9.49,0,0,1,26.75,168a17.35,17.35,0,0,1-1.28-4.78,39.22,39.22,0,0,1-.34-5.13,67.89,67.89,0,0,1,2.44-16.91,135.05,135.05,0,0,1,6.59-19.09,171,171,0,0,1,9.53-18.91A126.34,126.34,0,0,1,54.95,86.79,65.61,65.61,0,0,1,66.81,75.24q6-4.37,11.26-4.38a11.16,11.16,0,0,1,6.29,1.91A17.61,17.61,0,0,1,89.67,79a47.87,47.87,0,0,0,4.63,7.27,5.41,5.41,0,0,0,9,.23Z"/><path  d="M294.2,143.54a2.7,2.7,0,0,0-2.49-1.47,3.87,3.87,0,0,0-1.73.44,16.92,16.92,0,0,0-2,1.16h0q-10.09,9-17,14.94t-11.37,9.46a63.11,63.11,0,0,1-7,4.95,8.74,8.74,0,0,1-3.77,1.43,3.11,3.11,0,0,1-2.26-.67A2.38,2.38,0,0,1,246,172q.08-.79,2-3.08t4.74-5.53q2.83-3.26,6.07-7.23t6-8.16a57,57,0,0,0,4.63-8.34,19.27,19.27,0,0,0,1.85-7.81,9.59,9.59,0,0,0-1.2-4.86,11,11,0,0,0-3.13-3.48,14.25,14.25,0,0,0-4.37-2.09,17.43,17.43,0,0,0-4.93-.72,18.6,18.6,0,0,0-7.23,1.51,38.72,38.72,0,0,0-7.08,3.93,62.52,62.52,0,0,0-6.71,5.4c-2.16,2-4.17,4-6,6a12.45,12.45,0,0,0,.49-1.69c.17-.77.33-1.59.49-2.45s.27-1.68.38-2.45a14.32,14.32,0,0,0,.15-1.69,5.56,5.56,0,0,0-1.36-4.06,5.91,5.91,0,0,0-4.37-1.38,4.74,4.74,0,0,0-3.65,2.09,24.62,24.62,0,0,0-3.05,4.95,62.06,62.06,0,0,0-2.41,5.8q-1.05,2.95-1.66,4.56c-.68,2.43-1.46,4.91-2.3,7.44a3.33,3.33,0,0,0-1.15.76c-1.41,1.43-3,3-4.79,4.68s-3.66,3.38-5.61,5.09-3.93,3.33-5.91,4.91-3.89,3-5.73,4.2A39.9,39.9,0,0,1,185,171.3a9.71,9.71,0,0,1-4,1.12,1.81,1.81,0,0,1-1.73-.8,4.42,4.42,0,0,1-.45-2.15,17.52,17.52,0,0,1,.79-4.86,49.79,49.79,0,0,1,1.62-4.78h0q1.87-4.82,4.5-11.38t5.57-13.87l6.1-15q3.13-7.72,6.1-14.85t5.57-13.2q2.59-6.06,4.48-10.26,1-2.05,1.62-3.84a10.05,10.05,0,0,0,.64-3.39c0-2.26-.64-3.9-1.92-4.91a6.72,6.72,0,0,0-4.26-1.51A8.73,8.73,0,0,0,205,69a9.6,9.6,0,0,0-3.66,4.91l-22.82,57.55q-2.27,5.7-5,12.58t-5.35,13.83a61.6,61.6,0,0,0-2.51,8.1q-.94.78-1.89,1.5a39.54,39.54,0,0,1-7.27,4.5,14.54,14.54,0,0,1-5.8,1.61q-3.32,0-3.32-3.21a31.32,31.32,0,0,1,1-7.54,84,84,0,0,1,2.56-8.3q1.54-4.23,3.35-8.34t3.35-7.72c1-2.41,1.88-4.56,2.56-6.43a13.85,13.85,0,0,0,1-4.42,7.83,7.83,0,0,0-1.58-4.91,5.13,5.13,0,0,0-4.29-2.05,5.21,5.21,0,0,0-4.14,2,19.57,19.57,0,0,0-2.94,4.91A64.7,64.7,0,0,0,146,134a50.83,50.83,0,0,1-2.52,6.51q-2,4.55-5.09,10a75.88,75.88,0,0,1-6.82,10.26,41.29,41.29,0,0,1-8.21,8A15.69,15.69,0,0,1,114,172a2.64,2.64,0,0,1-2.49-1.34,5.91,5.91,0,0,1-.75-2.95,34.71,34.71,0,0,1,1.28-7.9,76.32,76.32,0,0,1,2.71-8q1.58-3.92,3.43-7.58t3.5-6.83q1.65-3.17,2.9-5.62a19.51,19.51,0,0,0,1.62-4,6,6,0,0,0,.23-1.69,4.58,4.58,0,0,0-.61-2.37,5.25,5.25,0,0,0-1.58-1.69,7.31,7.31,0,0,0-2.11-1,8.38,8.38,0,0,0-2.19-.31,7.57,7.57,0,0,0-4.48,1.47,12.1,12.1,0,0,0-3.65,4.86q-6.17,12.86-9.56,21.86t-5,14.67a57.11,57.11,0,0,0-1.92,8.3c-.2,1.75-.3,2.66-.3,2.72A16.76,16.76,0,0,0,96.08,181a10.68,10.68,0,0,0,2.79,4.06,10.06,10.06,0,0,0,4,2.14,18.06,18.06,0,0,0,4.79.62A27.58,27.58,0,0,0,117.85,186a33.15,33.15,0,0,0,8.44-4.77,42.89,42.89,0,0,0,6.71-6.51,48.74,48.74,0,0,0,4.9-7.05,44.87,44.87,0,0,0-1.2,9.55q0,5.81,2.26,8.21a7.71,7.71,0,0,0,5.88,2.41,20.31,20.31,0,0,0,8.14-2,54.55,54.55,0,0,0,9.19-5.31c.89-.63,1.77-1.29,2.65-2,.08.55.17,1.09.29,1.61a10.68,10.68,0,0,0,1.77,4,8.59,8.59,0,0,0,3.32,2.68,12,12,0,0,0,5.16,1,22.69,22.69,0,0,0,9.31-2.14,52,52,0,0,0,9.64-5.76,100.09,100.09,0,0,0,9.75-8.34l.5-.49c-.53,1.22-1.05,2.44-1.6,3.66q-3.87,8.79-8.09,17.76t-8.29,17.89c-.4,1.08-.84,2.19-1.32,3.35s-.94,2.34-1.39,3.56a33.52,33.52,0,0,0-1.13,3.7,15.69,15.69,0,0,0-.45,3.66q0,3.65,1.77,4.95a7.47,7.47,0,0,0,4.49,1.29,7.74,7.74,0,0,0,3.8-.93,10.6,10.6,0,0,0,3-2.5,17.76,17.76,0,0,0,2.33-3.62,33.39,33.39,0,0,0,1.77-4.38c.15-.54.5-1.55,1.06-3s1.2-3.2,1.92-5.17,1.5-4,2.33-6.25,1.62-4.3,2.37-6.29,1.42-3.73,2-5.22,1-2.5,1.25-3,.54-1.17,1-2.27l1.62-3.75q.9-2.09,2-4.46c.73-1.57,1.42-3.1,2.08-4.59s1.25-2.83,1.8-4,1-2.11,1.28-2.77q1.5-3.13,3.65-6.83c1.43-2.47,3-4.91,4.74-7.36a88.12,88.12,0,0,1,5.57-7.05,56.15,56.15,0,0,1,6.1-6,30.79,30.79,0,0,1,6.32-4.19,14.12,14.12,0,0,1,6.21-1.56,3.9,3.9,0,0,1,2.49.62A2.08,2.08,0,0,1,256,138a9.48,9.48,0,0,1-1.81,4.64,71.65,71.65,0,0,1-4.56,6.29q-2.76,3.44-6,7.27t-6.1,7.58a73.1,73.1,0,0,0-4.9,7.23,16.2,16.2,0,0,0-2.33,6.15,4.65,4.65,0,0,0-.08.76v.67a8.32,8.32,0,0,0,3.13,6.87q3.13,2.49,8.85,2.5a28.46,28.46,0,0,0,11.45-2.32,54,54,0,0,0,11-6.55,155.44,155.44,0,0,0,11.94-10.13q6.32-5.88,14.38-13a10,10,0,0,0,3.05-4,12,12,0,0,0,1-4.73A7.25,7.25,0,0,0,294.2,143.54Z"/>
      </symbol>
      <symbol id="icon-adobe" viewBox="0 0 300 300">
        <path d="M278.08,145.79v8.59c-.08.47-.18.92-.24,1.39-.44,3.74-.73,7.5-1.35,11.21a91.69,91.69,0,0,1-12.79,33.62,95.61,95.61,0,0,1-39.64,35.92,92.72,92.72,0,0,1-30.58,9c-2.36.27-4.72.52-7.09.78H94.73a11.47,11.47,0,0,0-1.39-.25,78.18,78.18,0,0,1-26.62-6.4A81.19,81.19,0,0,1,25.5,200.48a77.07,77.07,0,0,1-7.61-25.35q-.39-3.22-.77-6.43v-8.34a11.14,11.14,0,0,0,.25-1.39,76.8,76.8,0,0,1,5-23.26A81.15,81.15,0,0,1,63,91a79.63,79.63,0,0,1,31.83-8.23,88.42,88.42,0,0,1,16.35.75,2.06,2.06,0,0,0,1.54-.5,96.67,96.67,0,0,1,26.72-19.36,93.51,93.51,0,0,1,30.73-9.06c2.53-.29,5.08-.53,7.61-.79h7.82c.51.08,1,.17,1.54.23,3.87.45,7.77.73,11.6,1.38a91.94,91.94,0,0,1,33.42,12.69,95.62,95.62,0,0,1,36.09,39.71,92.5,92.5,0,0,1,9,30.6c.28,2.45.52,4.9.78,7.35M132.17,90.19c.16.21.2.3.26.34a104.41,104.41,0,0,1,15.33,9.27,1.61,1.61,0,0,0,1.43-.1,60.23,60.23,0,0,1,80.11,13.79c10.68,13.7,14.75,29.19,11.8,46.42a59.51,59.51,0,0,1-14.47,30.59c-14.35,15.82-32.31,22.37-53.46,19.6a56.45,56.45,0,0,1-33.56-16.72c-11.94-12.06-24-24-35.93-36.07A9.69,9.69,0,0,0,98,154.12a9,9,0,0,0-9.39,5.12c-1.55,3.22-1.35,6.85,2,10.15,13.35,13.1,26.58,26.33,39.91,39.45a64.45,64.45,0,0,0,7.32,6.23c9.74,7.07,20.72,11,32.62,12.56a78.68,78.68,0,0,0,27.79-1.1,78.21,78.21,0,0,0,59.92-91.17,74.91,74.91,0,0,0-17.62-36.12c-17.58-19.75-39.71-28.56-66-26.59a73.36,73.36,0,0,0-28.72,8.45,75.79,75.79,0,0,0-13.65,9.09M125.73,228l-1.08-1c-5.46-5.15-10.95-10.27-16.34-15.49a4.42,4.42,0,0,0-3.48-1.47c-5.46.13-10.93.16-16.31-1.06-26.76-6.05-42.3-35.15-32.44-60.75,7.78-20.19,29.32-32.48,50.55-28.69a44.24,44.24,0,0,1,24.73,12.8c8.36,8.63,17,17,25.43,25.57a7.6,7.6,0,0,0,6.8,2.45c3.45-.49,6.19-2.13,7.43-5.51a8.14,8.14,0,0,0-1.89-9c-8.87-9-17.54-18.2-26.77-26.81-17.9-16.7-39-21.67-62.37-14.56-31.41,9.54-49.79,42-42.61,74A63.24,63.24,0,0,0,99.27,228.1h25c.41,0,.82,0,1.46-.07"/>
      </symbol>
      <symbol id="icon-css" viewBox="0 0 300 300">
        <path d="M40.38,40.19l20.4,221L149.2,285l88.42-23.81L258,40.19ZM216.9,85.13l-2.7,26.69h0l-65.08,27.83-.15.07H211.9l-7.2,82.78-55.49,16.2-.1,0v0L93.34,222.2l-3.6-41.69h27.69l1.8,21.59,29.64,7.48.25-.07v0l30.69-8.64,2.1-34.79-32.79-.1h0l-61.18-.2h0l-2.1-26.09,63.28-26.36h0l3.69-1.54H83.44l-3.3-26.69H216.9Z"/>
      </symbol>
      <symbol id="icon-html" viewBox="0 0 300 300">
        <path d="M41.59,25.28,61.77,251.62l90.55,25.14,90.8-25.17,20.2-226.3ZM221.24,79,220,93.07l-.56,6.24H113.19l2.54,28.43H216.89l-.68,7.45-6.52,73.08-.42,4.69L152.46,228.7h0l-.13,0L95.47,213l-3.89-43.59h27.87l2,22.14,30.92,8.35h0l31-8.36,3.22-36H90.34L83.52,79l-.67-7.45h139Z"/>
      </symbol>
      <symbol id="icon-git" viewBox="0 0 300 300">
        <path d="M274.06,137.49,159.31,22.74a16.93,16.93,0,0,0-23.94,0L111.54,46.57,141.77,76.8a20.1,20.1,0,0,1,25.46,25.63l29.13,29.13a20.12,20.12,0,1,1-12.06,11.36l-27.17-27.17v71.5a20.35,20.35,0,0,1,5.33,3.81,20.12,20.12,0,1,1-21.89-4.39V114.49a19.9,19.9,0,0,1-6.59-4.4,20.13,20.13,0,0,1-4.34-22l-29.8-29.8L21.14,137a16.93,16.93,0,0,0,0,23.94L135.9,275.65a16.93,16.93,0,0,0,23.94,0L274.06,161.43a16.94,16.94,0,0,0,0-23.95"/>
      </symbol>
      <symbol id="icon-jquery" viewBox="0 0 300 300">
        <path d="M266.06,207.05,265,209.81c-18.12,35.29-50.23,56.93-96,57.89a134.26,134.26,0,0,1-55.13-11c-28.76-12.92-52.95-36.88-66.16-62.49-18.95-36.7-22.37-87.75,17-125.89.61,0-4.78,8.27-4.17,8.27L59.45,80.1C31.06,169,118.54,245.17,203.57,236.46c20.44-2.09,46.5-13.29,62.48-29.4"/><path  d="M214.6,170.3c21.39.15,43.74-9.85,57-23.89-11.58,29.39-45.1,48.28-90,42.27-37.64-5-73.49-42.91-77.19-80.86-2.7-27.62,6.92-44.1,22.79-64.1-5.51,10.46-7.57,18.52-8.09,23.67C113,127.13,166.76,170,214.6,170.3"/><path d="M269.42,96.09c-3.32,7.63-22.47,22-30.93,23.84-33.48,7.13-54.93-8.74-66.16-26.81A82.75,82.75,0,0,1,167,81.16c-4.36-14.81-2.35-37.22,10.64-48.81-3.91,11-4.49,23.84-2.43,32.16a71.42,71.42,0,0,0,8.86,19.35c6.92,10.07,11.32,12.52,20.21,17.87a53.63,53.63,0,0,0,10.16,4.87c8.44,2.48,26.24,9,55-10.51"/>
      </symbol>
      <symbol id="icon-sentry" viewBox="0 0 300 300">
        <path  d="M260.74,216.55c5.35,9.29,5.87,19.06,1.42,26.78S249,255.49,238.33,255.49H218.16c.24-3.76.4-7.56.4-11.38,0-4.28-.2-8.51-.49-12.72h13.27a5.91,5.91,0,0,0,5.9-5.92,6,6,0,0,0-.71-2.82l-84.58-147a5.88,5.88,0,0,0-10.17-.14L128.34,98.83a181.69,181.69,0,0,1,72,132.55c.29,4.21.49,8.43.49,12.72,0,3.82-.16,7.61-.39,11.38H134.56c.36-3.75.57-7.54.57-11.38a114.89,114.89,0,0,0-.72-12.72,115.65,115.65,0,0,0-39.31-74.8l-9.25,16.07a97.43,97.43,0,0,1,30.3,58.74A97.84,97.84,0,0,1,117,244.1a99,99,0,0,1-.68,11.38H55.25c-10.71,0-19.39-4.43-23.83-12.16s-3.93-17.48,1.42-26.78L45.38,195a73.45,73.45,0,0,1,19.16,14.84l-7.47,12.73a6,6,0,0,0-.71,2.82,5.9,5.9,0,0,0,5.89,5.91H91.87a73.34,73.34,0,0,0-18.42-37.16,73.8,73.8,0,0,0-19.18-14.86l33.08-57.47A140,140,0,0,1,107.3,135.4a139.73,139.73,0,0,1,51.25,96h17.73a157.49,157.49,0,0,0-60.05-111.48,158.33,158.33,0,0,0-20-13.37l28.24-49.05c5.34-9.3,13.51-14.63,22.41-14.63s17.05,5.34,22.4,14.63Z"/>
      </symbol>
      <symbol id="icon-angular" viewBox="0 0 300 300">
        <polygon points="133.08 152.96 155 152.96 155 152.96 155 152.96 176.92 152.96 155 100.22 133.08 152.96"/><path d="M155,17h0L34.94,59.86,53.25,218.6,155,275h0L256.75,218.6,275.06,59.86Zm74.92,196.79h-28l-15.09-37.65h-63.7l-15.09,37.65h-28L155,45.54h0l74.92,168.29Z"/>
      </symbol>
      <symbol id="icon-webpack" viewBox="0 0 300 300">
        <path d="M265.64,217.44,159.3,277.6V230.74l66.26-36.45Zm7.27-6.59V85L234,107.51v80.86l38.92,22.49Zm-229.3,6.59L150,277.6V230.74L83.7,194.3,43.62,217.44Zm-7.27-6.59V85l38.92,22.46v80.86L36.34,210.85ZM40.89,76.91,150,15.2V60.5L80.09,98.94l-.53.31Zm227.47,0L159.3,15.2V60.5l69.87,38.43.53.31Z"/><path d="M150,220.09,84.59,184.15V112.93L150,150.67v69.41Zm9.33,0,65.37-35.94V112.93L159.3,150.67v69.41ZM89,104.69l65.62-36.09,65.62,36.09-65.62,37.9Z"/>
      </symbol>
      <symbol id="icon-js" viewBox="0 0 300 300">
        <path  d="M143.5,8.3a23.53,23.53,0,0,1,23,0q52.45,29.64,104.9,59.25a21.59,21.59,0,0,1,10.91,18.6V205a21.83,21.83,0,0,1-11.72,19Q218.28,253.49,166,283a22.86,22.86,0,0,1-23.22-.65q-15.67-9.09-31.36-18.13c-2.14-1.27-4.54-2.29-6-4.36,1.33-1.8,3.72-2,5.65-2.8a73.2,73.2,0,0,0,12.37-5.77,2.89,2.89,0,0,1,3.22.19c8.93,5.12,17.79,10.39,26.75,15.47,1.91,1.1,3.85-.36,5.48-1.27q51.3-29,102.63-57.92a3.39,3.39,0,0,0,1.86-3.34q0-58.82,0-117.64a3.68,3.68,0,0,0-2.19-3.67Q209.1,53.76,157,24.39a3.62,3.62,0,0,0-4.09,0Q100.86,53.73,48.79,83.11a3.63,3.63,0,0,0-2.2,3.65q0,58.82,0,117.65a3.33,3.33,0,0,0,1.89,3.3q13.9,7.88,27.82,15.71c5.22,2.81,11.64,4.48,17.39,2.33a13.19,13.19,0,0,0,8.54-12.4c0-39,0-78,0-116.95a2.94,2.94,0,0,1,3.2-3c4.45,0,8.91-.06,13.36,0A3.08,3.08,0,0,1,121.73,97c0,39.23,0,78.46,0,117.69,0,10.45-4.28,21.83-14,26.95-11.91,6.17-26.64,4.86-38.41-1.06C59.15,235.47,49.43,229.47,39.42,224A21.78,21.78,0,0,1,27.7,205V86.15A21.63,21.63,0,0,1,39,67.32Q91.25,37.82,143.5,8.3Z"/><path  d="M173.9,90.64c15.2-1,31.47-.58,45.14,6.91,10.59,5.74,16.46,17.78,16.64,29.54a3.2,3.2,0,0,1-3.47,2.35c-4.41,0-8.82.06-13.23,0-1.87.07-3-1.65-3.19-3.31-1.27-5.63-4.34-11.2-9.63-13.92-8.13-4.07-17.56-3.87-26.43-3.78-6.47.34-13.43.9-18.92,4.71-4.21,2.88-5.49,8.7-4,13.39,1.42,3.37,5.3,4.45,8.48,5.45,18.31,4.79,37.72,4.31,55.69,10.62,7.44,2.57,14.71,7.57,17.26,15.35,3.33,10.44,1.87,22.91-5.56,31.29-6,6.9-14.79,10.65-23.54,12.69-11.64,2.59-23.71,2.66-35.53,1.51-11.11-1.27-22.68-4.19-31.25-11.76-7.34-6.37-10.92-16.29-10.56-25.87a3,3,0,0,1,3.25-2.61c4.44,0,8.88,0,13.32,0a3.11,3.11,0,0,1,3.18,3.08c.82,5.36,2.83,11,7.51,14.17,9,5.82,20.35,5.42,30.68,5.59,8.56-.38,18.17-.49,25.16-6.15,3.69-3.23,4.78-8.63,3.78-13.27-1.08-3.92-5.18-5.75-8.7-6.94-18.08-5.72-37.7-3.64-55.61-10.11-7.27-2.57-14.3-7.43-17.09-14.89-3.9-10.57-2.11-23.64,6.09-31.74,8-8.05,19.55-11.15,30.52-12.26Z"/>
      </symbol>
    </svg>
  </article>
</template>

<script>
import LoadProject from '@/components/LoadProject';
import LoadImage from '@/components/LoadImage';

export default {
  components: {
    LoadProject,
    LoadImage,
  },
  props: ['projectPass'],
  mounted() {
    setTimeout(() => {
      this.show = true;
      this.getWindowWidth();
      window.addEventListener('resize', this.getWindowWidth, false);
      window.addEventListener('hashchange', this.hashChange, false);
      window.location.hash = `/${this.projectPass.name}`;
      setTimeout(() => {
        this.eleTop = this.$el.querySelector('.js-elementTop').offsetTop;
        this.stickInfo();
      }, 1000);
    }, 1500);
  },
  methods: {
    hideProject() {
      this.$emit('projectHide');
    },
    hashChange() {
      if (window.location.hash === '') {
        this.$emit('projectHide');
      }
    },
    toTop() {
      if (window.scrollY !== 0) {
        setTimeout(() => {
          window.scrollTo(0, window.scrollY - 100);
          this.toTop();
        }, 10);
      }
    },
    getWindowWidth() {
      this.windowWidth = window.innerWidth;
      if (this.windowWidth < 768) {
        this.isPhone = true;
        this.isFixed = false;
      } else {
        this.isPhone = false;
      }
    },
    stickInfo() {
      window.addEventListener('scroll', () => {
        if (!this.isPhone) {
          if (!this.isFixed && this.eleTop - window.scrollY < 0) {
            this.isFixed = true;
          } else if (this.isFixed && window.scrollY < this.eleTop) {
            this.isFixed = false;
          }
        } else {
          this.isFixed = false;
        }
      });
    },
    loadImageSend(src) {
      this.loadImageSrc = src;
      this.loadImageShow = true;
    },
    loadImageHide() {
      this.loadImageShow = false;
      this.loadImageSrc = '';
    },
  },
  data() {
    return {
      loading: true,
      projectSelected: false,
      show: false,
      isFixed: false,
      windowWidth: 0,
      isPhone: false,
      eleTop: 0,
      loadImageShow: false,
      loadImageSrc: '',
    };
  },
};
</script>

<style lang="scss">
  @import "../scss/utilities/base";
  @import "../scss/components/stem-scroll";
  @import "../scss/components/icon-close";

  $left: 30%;
  $space: 50px;

  .project {

    &__header {
      min-height: 50vh;
      overflow: hidden;
      background-color: $color-white;
      padding: theme-get(space, small);
      background-image: url('/static/img/logo-opacity.svg');
      background-position: center;
      background-size: 120%;
      background-repeat: no-repeat;
      img {
        min-width: 100%;
        display: block;
      }
      h1 {
        font-size: theme-get(text, size, massive);
        font-weight: theme-get(weight, light);
        text-align: center;
        margin-top: 25vh;
        line-height: 1;

      }
      h2 {
        font-size: theme-get(text, size, huge);
        font-weight: theme-get(weight, light);
        text-align: center;
        margin-bottom: 15vh;
        line-height: 1;
        color: $color-salmon;
      }
      &__link {
        background: rgba(0,0,0,0.2);
        color: $color-lt-gray;
        margin: - theme-get(space, small);;
        padding: theme-get(space, small);;
        display: block;
        text-align: center;
        text-decoration: none;
        transition: background-color 0.3s ease-out;
        &:hover:not(span) {
          background-color: rgba(0,0,0,0.4);
        }
      }
    }
    &__body {
      padding-bottom: 50vh;
      &__info {
        position: absolute;
        width: $left;
        float: left;
        margin-right: $space;
        &.fixed {
          position: fixed;
          top: 0;
        }
        ul {
          padding: theme-get(space, normal);
        }
        li {
          margin-bottom: theme-get(space, big);
          text-align: right;
        }
        p {
          font-size: theme-get(text, size, bigger);
          font-weight: theme-get(weight, light);
          line-height: 1;
        }
        @include phone {
          position: static;
          width: 100%;
          margin-right: 0;
          li {
            text-align: left;
          }
        }
      }
      &__description {
        margin-left: $left;
        padding-left: $space;
        @include phone {
          margin-left: 0;
          padding-left: 0;
        }
        &__block {
          position: relative;
          padding-bottom: theme-get(space, big);
           > * {
             margin-bottom: 0;
           }
           &__content {
             padding-left: theme-get(space, normal);
             padding-right: theme-get(space, normal);
             padding-top: theme-get(space, small);
           }
        }
        p {
          font-size: theme-get(text, size, big);
          font-weight: theme-get(weight, light);
        }
        h3 {
          line-height: 1;
        }
        legend {
          padding-top: theme-get(space, small);
          padding-bottom: theme-get(space, small);
          border-top: 1px solid $color-lt-gray;
        }
      }
    }

    legend {
      width: 100%;
      text-transform: uppercase;
      color: theme-get(text, color, blue);
      font-size: theme-get(text, size, small);
    }
    &__palette {
      margin-bottom: theme-get(space, big);
      li {
        border: 10px solid $color-white;
        border-bottom-width: 40px;
        border-radius: 10px;
        display: inline-block;
        margin-right: theme-get(space, small);
        position: relative;
        @include small {
          width: 80px;
          height: 80px;
          border-top-width: 7px;
          border-right-width: 7px;
          border-bottom-width: 40px;
          border-left-width: 7px;
        }
        @include large {
          width: 120px;
          height: 120px;
        }
        span {
          position: absolute;
          bottom: -35px;
          width: 100%;
        }
      }
    }
    &__type {
      margin-bottom: theme-get(space, big);
      font-size: theme-get(text, size, huge);
      line-height: 1;
      li {
        @extend %text-overflow;
      }
    }
    &__stack {
      margin: auto;
      li {
        display: inline-block;
        width: 120px;
        padding: theme-get(space, small);
        font-size: theme-get(text, size, root);
        text-align: center;
        @include phone {
          width: 110px;
        }
      }
      svg {
        width: theme-get(icon-size, small);
        height: theme-get(icon-size, small);
        fill: $color-gray;
      }
      p {
        font-size: theme-get(text, size, small);
      }
    }
    &__screens {
      overflow: hidden;
      li {
        float: left;
        a {
          display: block;
          padding: theme-get(space, small);
          box-shadow: 0 0 30px rgba(0,0,0,0.2);
          transition: box-shadow 0.4s ease-out;
          &:hover {
            box-shadow: 0 0 30px rgba(0,0,0,0.7);
          }
          @include phone {
            max-width: 130px;
          }
          @include tablet {
            max-width: 150px;
          }
          @include small {
            margin: theme-get(space, normal);
          }
          @include large {
            margin: theme-get(space, big);
            max-width: 250px;
          }
        }
        img {
          max-width: 100%;
        }
      }
    }
  }

  .effect-blur {
    will-change: filter, boder;
    transition: all 0.3s;
    &--on {
      filter: blur(3px);
      .stem__sprite {
        border-left: 20px solid $color-light;
        border-right: 20px solid $color-light;
        margin-left: -70px;
      }
    }
  }

  .grow-enter-active, .grow-leave-active {
    h1 {
      transition: all 0.8s;
    }
    h2 {
      transition: all 0.8s;
      transition-delay: 0.5s;
    }
    transition: all 0.8s
  }
  .grow-enter, .grow-leave-to {
      min-height: 100vh;
      h1 {
        margin-top: 0;
      }
      h2 {
        opacity: 0;

      }
  }
</style>
